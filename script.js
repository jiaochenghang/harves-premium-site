...
const tryPlayPlayerVideo = async (video) => {
  if (
    !video ||
  ...
  try {
    ...
    if (video.dataset.started !== "true") video.currentTime = 0;
    await video.play();
    video.dataset.started = "true";
    video.classList.add("is-playing");
  } catch {
    video.dataset.started = video.currentTime > 0 ? "true" : "false";
  } finally {
...
playerVideos.forEach((video) => {
  ...
  video.addEventListener("error", () => video.classList.add("is-ready"));
  playerVideoObserver.observe(video);
});

const retryVisiblePlayerVideos = () => {
  playerVideos.forEach((video) => tryPlayPlayerVideo(video));
};

document.addEventListener("touchstart", retryVisiblePlayerVideos, { passive: true });
document.addEventListener("WeixinJSBridgeReady", retryVisiblePlayerVideos);
